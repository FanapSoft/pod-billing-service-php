...
class BillingService extends BaseService
{
    ...
    public function getOtt($apiName = 'getOtt') {
        ...
        $validateResult = self::validateOption($apiName, $option);
        if ($validateResult['validate']) {
            return ApiRequestHandler::Request(
                self::$config[$this->serverType][self::$billingApi[$apiName]['baseUri']],
...
//        $header = array_filter(array_merge($this->header, (array) $header), 'self::filterNotEmptyValue');
        ...
        if ($validateResult['validate']) {
            ...
            if ($paramKey == 'query') {
                ...
                $relativeUri = self::$billingApi[$apiName]['subUri'] . "?" . $httpQuery;
    ...
    public function createPreInvoice($params) {
        ...
        $baseUri =  self::$config[$this->serverType]["PRIVATE-CALL-ADDRESS"];
        ...
        if ($validateResult['validate']) {
            ...
            $preInvoiceUri = self::$config[$this->serverType]["PRIVATE-CALL-ADDRESS"]. "/v1/pbc/preinvoice/" . $result["Result"];
    ...
    public function getPayInvoiceByWalletLink($params) {
        ...
        $validateResult = self::validateOption($apiName, $option);
        if ($validateResult['validate']) {
            return self::$config[$this->serverType]["PRIVATE-CALL-ADDRESS"] . self::$billingApi[$apiName]['subUri'] . "?" . $httpQuery;
        }
        else {
            throw new Exception($validateResult['errorMessage'], self::VALIDATION_ERROR_CODE);
        }
        ...
        if ($validateResult['validate']) {
            ...
            foreach ($result['result'] as $key => $fileInfo){
                if ($fileInfo['statusCode'] === 'EXPORT_SERVICE_STATUS_SUCCESSFUL'){
                    ...
                    $result['result'][$key]['downloadPath'] = self::$config[$this->serverType]["FILE-SERVER-ADDRESS"] ."/nzh/file/?fileId=$fileId&hashCode=$hashCode";
                }
            }
            return $result;
        }
    ...
    public function issueMultiInvoice($params) {
        ...
        if ($validateResult['validate']) {
            ...
            // if we have array type fields httpQuery add index but pod server dont accept it so we should remove index from http query
            if ($paramKey == 'query') {
                $httpQuery = self::buildHttpQuery($option[$paramKey]);
                $relativeUri = self::$billingApi[$apiName]['subUri'] . "?" . $httpQuery;
                unset($option['query']); // unset query because it is added to uri and dont need send again in query params
            }
            return  ApiRequestHandler::Request(
